...
var ajax;
function ajaxInvoke(url,process){
	//异步请求对象
...
function process(){
	...
	if(ajax.readyState == 4){ //4代表异步请求完成
		if(ajax.status == 200){ //200代表服务器端相应正常
			var aj = ajax.responseText;
			if(aj != 0){
				
				var msgBox = document.getElementById("uName");
				msgBox.style.display = "inline";
				msgBox.innerHTML = "用户名已经存在";
				return false;
			}else{
				var msgBox = document.getElementById("uName");
				msgBox.style.display = "inline";
...
function Checkexist(){
	...
	var url = "CheckUserName?userName=" + userName.value;
	ajaxInvoke(url,process);
}
